...
public class ConnectorTestApp {
    ...
    public void run() throws Exception {
        ...
        try {
             ...
             * RdfUtils.resourceToString( forum, Syntax.Turtle ) ); if (
             * structureReader.hasChildContainer( forum.getResource() .asURI() )
             * ) { List<Container> threads = structureReader .listContainer(
             * forum.asURI() ); for ( Container thread : threads ) {
             * System.out.println( RdfUtils.resourceToString( thread,
             * Syntax.Turtle ) ); if ( postReader.hasPosts(
             * thread.getResource().asURI() ) ) { List<Post> posts =
             * postReader.pollPosts( thread .asURI(), null, -1 ); for ( Post
             * post : posts ) { System.out.println( RdfUtils.resourceToString(
             * post, Syntax.Turtle ) ); } } } } }
             */
