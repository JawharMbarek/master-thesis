	...
	public void init() {
		...
		try {
			while ( accountIterator.hasNext() ) {
				Resource resource = accountIterator.next();
		...
		try {
			while ( serviceIter.hasNext() ) {
				Resource resource = serviceIter.next();
				...
				if ( null != entry ) {
					...
					for ( int i = 0; i < view.getConnectorClassCombobox().getItemCount(); i++ ) {
						...
						if ( classEntry.getClazz().getName()
						        .equals( connectorConfig.getConnectorClassName() ) ) {
	...
	private void updateConnectorConfigList() {
		...
		try {
			while ( conConfigIterator.hasNext() ) {
				Resource resource = conConfigIterator.next();
	...
	public void actionPerformed( ActionEvent e ) {
		...
		if ( ConnectorConfigView.ACTION_CMD_URI_GENERATE.equals( cmd ) ) {
			...
			if ( null != cfg ) {
				...
				cfg.setConnectorClassName( clazz.getClazz().getName() );
