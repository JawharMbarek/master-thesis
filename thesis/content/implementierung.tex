...
Da Beiträge in Moodle in einer Forumsstruktur geschrieben werden, ist das Mapping nach SIOC, wie in Tabelle \ref{tbl:klassen_mapping_sioc_moodle} zu sehen, problemlos möglich. Bei dem Objekt der Klasse Service, die den Moodle Service beschreibt, ist es wichtig dass die Eigenschaft \texttt{siocs:serviceEndpoint} mit der URI zur verwendeten Moodleinstanz gesetzt ist. Diese ist für die Funktion 

\begin{table}[ht]
    \centering
    \caption{Mapping zwischen SIOC und Moodle Klassen}
    \begin{tabular}{l|p{11cm}}
        \textbf{SIOC Klasse} & \textbf{Moodle Klasse} \\ 
        \hline
        UserAccount & User \\
        Forum & Forum\\
        Thread & ForumDiscussionTopic\\
        Post & ForumPost\\
    \end{tabular}
    \label{tbl:klassen_mapping_sioc_moodle}
\end{table}

Die Tabelle \ref{tbl:moodle_sioc_uris} zeigt das Aussehen der URIs, mit denen die Objekte der SIOC-Klassen in RDF benannt werden.
